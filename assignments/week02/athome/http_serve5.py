...
def resolve_uri(uri):
    """takes a URI and returns a filepath if exists, else returns 404"""
    path = 'web'
...
def parse_request(request, host, date):
	...
	return client_error_response(405, ndata[0], host, date)
        ...
        return client_error_response(400, ndata[2], host, date)   

def notFound_response(uri, host, date):
    """Returns 404 Not Found Error if uri not found"""
    body = getFile("404ErrorPage.html", 'r')
...
def getFile(request, readType):
    """Returns the requested file"""
    print request
    file = open(request, readType)
    fileStuff = file.read()
    file.close()
    return fileStuff
	...
	#print uri
	    ...
	    uri_response = resolve_uri(uri)
            if uri_response != 404:
                ...
                response = getFile(uri_response, 'r' if get_maintype(type) == "text" else 'rb')
                newData = ok_response(response, serverName, type, date)
	    else:
